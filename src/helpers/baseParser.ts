 ...
 */
export function getPropValue(val: any) {
  if (Array.isArray(val)) {
    // 数组，则继续深度递归
    return getPropValueFromArray(val);
  } else if (isJsonSchema(val)) {
    // schema，则转为组件
    return objectSchemaToComponent(val);
...
// 获取所有属性
export function getProps(props: Record<any, any>) {
  const res: Record<any, any> = {};

  // 遍历组件属性，并转换
  Object.keys(props).forEach(key => {
    const val = props[key];
    res[key] = getPropValue(val);
  });
...
export function baseSchemaParser(
  ...
  // 转换 props
  const componentProps = getProps(originProps);
  ...
  if (typeof component === 'string' && getComponent(component)) {
    return createElement(getComponent(component), componentProps);
  }
